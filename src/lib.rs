...
//! ```
//!
//! Note: you can use `ARBTEST_BUDGET_MS` to adjust time budget without
//! recompilation.
//!
//! Note: while we are waiting for the docs, studying the source might be
//! helpful, it's short!
...
impl Builder {
    pub fn start_size(mut self, size: u32) -> Builder {
        self.start_size = size;
        self
    }
    pub fn end_size(mut self, size: u32) -> Builder {
        self.end_size = size;
        self
    }
    ...
    pub fn run(mut self, prop: Property) {
        ...
        'search: loop {
            ...
            let bigger = (size as u64) * 5 / 4;
            size = if bigger < self.end_size as u64 { bigger as u32 } else { self.end_size };
        }
    }
